...
function updateDOMProperties(node, prevVNode, nextVNode) {
    ...
    //remove attributes
    Object.keys(prevVNode.attributes || []).forEach((key, _) => {
        node.removeAttribute(key);
    });

    //remove old event listeners
    Object.keys(prevVNode.events || []).forEach((key, _) => {
    ...
    //add event listeners
    Object.keys(nextVNode.events || []).forEach((key, _) => {
...
const instantiate = (vNode) => {
    if (!vNode.tag) {
        ...
        const domNode =
            vNode.tag !== "TEXT_ELEMENT" ? document.createElement(vNode.tag) : document.createTextNode(vNode.nodeValue);
...
class List extends Component {
    ...
    initList(item, store, remove) {
        if (!(store instanceof CollectionStore))
            throw "Error unknown data store provided, please use a CollectionStore!";
        this.store = store;
...
class Router {
    ...
    match(route) {
        ...
        for (let [path, { pathRoute, handler, params }] of this.routes) {
            ...
            if (match !== null) {
                ...
                const routeParams = match.slice(1).reduce((allParams, value, index) => {
    ...
    on(...pageRoutes) {
        ...
        console.log(this.numRegistered, this.numRoutes);
...
class Reader {
    ...
    getNextWord(includeQuotes = false) {
        ...
        while (
            ...
            (!this.specialCharacters.has(this.currentChar) || (!includeQuotes && quoteCount === 1))
        ) {
...
const parseJSExpr = (reader, values, attribute) => {
    ...
    if (val) {
        ...
        if (val.startsWith("<")) {
            console.log(
                "Warning, attempting to inject HTML into the page, this is a security risk and should be avoided. ",
                val
            );
            return val;
        }
...
const parseCSStringToDict = (reader, dict, selector, values) => {
    ...
    while (reader.index < reader.length) {
        ...
        if (reader.currentChar === ":") {
            ...
            if (char !== "{") {
                ...
                if (value === "{") {
                    ...
                    if (constant !== CSS_PLACEHOLDER)
                        throw "Invalid JS expression while trying to parse the value of a key!";
                    value = values.shift();
        ...
        if (char === "{") {
            ...
            if (
                word.includes("@keyframes") ||
                word.includes("@media") ||
                dict["tag"].includes("@keyframes") ||
                dict["tag"].includes("@media")
            ) {
                newSelector = word;
            }
